...
class StompWebSocketService {
  ...
  void sendUpdateStatus({
    ...
    if (!_connected) {
      print('[WebSocket] ❌ Not connected. Cannot send status update.');
    ...
    final data = {
      ...
      "timestamp":
          DateTime.now().toIso8601String(), // Add timestamp for tracking
    };
    ...
    try {
      ...
      print('[WebSocket] ✅ Status update sent: $data');
    } catch (e) {
      print('[WebSocket] ❌ Error sending status update: $e');
      throw Exception('Failed to send status update via WebSocket: $e');
    }
  }
